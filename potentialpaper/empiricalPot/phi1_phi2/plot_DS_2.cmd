...
splot \
	...
	'+' using 1:($1+ 50)*-1:(-0.5) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
	'+' using 1:($1-130)*-1:(-0.5) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
	'+' using 1:($1+230)*-1:(-0.5) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
	'+' using 1:($1-310)*-1:(-0.5) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
...
splot FILE u ( shift_phi($1)):( shift_phi($2)):4 with pm3d,\
	'+' using 1:($1+ 50)*-1:(9) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
	'+' using 1:($1-130)*-1:(9) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
	'+' using 1:($1+230)*-1:(9) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
	'+' using 1:($1-310)*-1:(9) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
...
splot FILE u ( shift_phi($1)):( shift_phi($2)):(shift_z($3)) with pm3d,\
	'+' using 1:($1+ 50)*-1:(0) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
	'+' using 1:($1-130)*-1:(0) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
	'+' using 1:($1+230)*-1:(0) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
	'+' using 1:($1-310)*-1:(0) with lines lc rgb '#CCCCCC' dt 3 lw 1,\
...
plot \
	...
	sprintf("<cat %s | awk '{if ($2==-10) print }'",FILE) u 1:($5-2*(-6227.1749)) w l lt 2 dt 2,\
