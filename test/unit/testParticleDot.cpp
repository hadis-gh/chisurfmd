...
TEST_CASE("calTotalForce - three particles in a line with LJForceFunc (abs check)") {
    using T = double;
    ParticleDot<T> p1, p2, p3;
    p1.r = {{0.0, 0.0}};
    p2.r = {{1.0, 0.0}};
    p3.r = {{2.0, 0.0}};

    std::vector<ParticleDot<T>> particles{p1, p2, p3};
    ...
    // REQUIRE(f_p1.abs() == Approx(23.8183951327));
    // REQUIRE(f_p2.abs() == Approx(0.0));
    // REQUIRE(f_p3.abs() == Approx(23.8183951327));
...
TEST_CASE("calTotalForce - PBC adjustment applied with ForceFunc") {
    ...
    std::vector<ParticleDot<T>> particles{p1, p2};
    auto f_p1 = calTotalForce(p1, particles, areaL, ForceFunc);
    auto f_p2 = calTotalForce(p2, particles, areaL, ForceFunc);
